...
static void psvDebugScreenSetColors(void)
{
	...
	// foregound color
	if ((colors.fgIndex <= 7) && (colors.fgIntensity == 1))
	{ // ANSI palette with increased intensity
		colors.fgIndex |= 0x8;
	}
	else if ((colors.fgIndex <= 15) && (colors.fgIntensity != 1))
	{ // ANSI palette with standard/decreased intensity
		colors.fgIndex &= 0x7;
	}
	...
	{
		if ((colors.fgIndex <= 7) && (colors.fgIntensity == 2))
		{ // "ANSI" palette with decreased intensity
			*color_fg = DARK_COLORS_BGR[colors.fgIndex];
		}
		else
		{ // ANSI/VTERM/GREYSCALE palette
	...
	*color_fg |= 0xFF000000; // opaque
	...
	else if ((colors.bgIndex <= 15) && (colors.bgIntensity != 1))
	{ // ANSI palette with standard/decreased intensity
		colors.bgIndex &= 0x7;
	}
	...
	{
		if ((colors.bgIndex <= 7) && (colors.bgIntensity == 2))
		{ // "ANSI" palette with decreased intensity
...
static size_t psvDebugScreenEscape(const unsigned char *str)
{
	...
	for (i = 0, argc = 0; (argc < (sizeof(arg) / sizeof(*arg))) && (str[i] != '\0'); i++)
	{
		...
		case 'K':
			if (arg[0] == 0)
			{																						   // from cursor to end of line/screen
			...
			{																			  // from beginning of line/screen to cursor
		...
		case 'm':
			for (c = 0; c <= argc; c++)
			{
				...
				case 48: // background color
					...
					if (arg[c + 1] == 5)
					{ // 8-bit: [0-15][16-231][232-255] color map
